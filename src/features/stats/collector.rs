...
impl<'a> StatsCollector<'a> {
    ...
    pub fn new(client: &'a ThingsClient) -> Self {
        Self { client }
    }
    ...
    pub fn todos_by_tag<'b>(&self, todos: &'b [Todo]) -> HashMap<String, Vec<&'b Todo>> {
        ...
        for todo in todos {
            if todo.tags.is_empty() {
                by_tag.entry("(No Tags)".to_string()).or_default().push(todo);
            } else {
    ...
    pub fn overdue_todos<'b>(&self, todos: &'b [Todo]) -> Vec<&'b Todo> {
        ...
        todos
            ...
            .filter(|t| {
                if let Some(due) = t.due_date {
                    due < today && t.status == Status::Open
                } else {
                    false
                }
            })
